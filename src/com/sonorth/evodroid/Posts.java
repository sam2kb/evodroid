...
public class Posts extends FragmentActivity implements OnPostSelectedListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titleBar = (AppTitleBar) findViewById(R.id.postsActionBar);
		...
		titleBar.setOnBlogChangedListener(new OnBlogChangedListener() {
	     ...
	     b2evolution.setOnPostUploadedListener(new b2evolution.OnPostUploadedListener(){
	...
	protected void checkForLocalChanges() {
		boolean hasLocalChanges = b2evolution.DB.findLocalChanges();
		if (hasLocalChanges) {
			AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(
					Posts.this);
			dialogBuilder.setTitle(getResources().getText(
	...
	protected void onResume() {
		...
		attemptToSelectPost();
		postList.loadPosts(false);
		if (b2evolution.postsShouldRefresh) {
			checkForLocalChanges();
			b2evolution.postsShouldRefresh = false;
		}
	...
	public class deletePostTask extends AsyncTask<Post, Void, Boolean> {
		...
		protected Boolean doInBackground(Post... params) {
			...
			XMLRPCClient client = new XMLRPCClient(
					b2evolution.currentBlog.getUrl(),
			...
			Object[] pageParams = { b2evolution.currentBlog.getBlogId(),
					b2evolution.currentBlog.getUsername(),
					b2evolution.currentBlog.getPassword(), post.getPostid() };
	...
	public class shareURLTask extends AsyncTask<Post, Void, String> {
		...
		protected String doInBackground(Post... params) {
			...
			try {
				if (isPage) {
					Object[] vParams = { b2evolution.currentBlog.getBlogId(),
							post.getPostid(),
							b2evolution.currentBlog.getUsername(),
							b2evolution.currentBlog.getPassword() };
