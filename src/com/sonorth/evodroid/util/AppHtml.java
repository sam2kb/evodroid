...
public class AppHtml {
	...
	private static void withinParagraph(StringBuilder out, Spanned text,
		...
		for (int i = start; i < end; i = next) {
			...
			for (int j = 0; j < style.length; j++) {
				...
				if (style[j] instanceof AppUnderlineSpan) {
					out.append("<u>");
				}
				...
				if (style[j] instanceof ImageSpan) {
					out.append("<img src=\"");
					out.append(((AppImageSpan) style[j]).getSource());
					out.append("\" android-uri=\""
							+ ((AppImageSpan) style[j]).getImageSource()
									.toString() + "\"");
					out.append(" />");
	...
	private static void processImage(StringBuilder out, Spanned text,
			int start, int end) {
		int next;
...
class HtmlToSpannedConverter implements ContentHandler {
	...
	private AppHtml.ImageGetter mImageGetter;
	...
	public HtmlToSpannedConverter(String source,
			AppHtml.ImageGetter imageGetter, AppHtml.TagHandler tagHandler,
	...
	private static void startImg(SpannableStringBuilder text,
			Attributes attributes, AppHtml.ImageGetter img) {
		...
		if (mediaData != null) {
			...
			AppImageSpan is = new AppImageSpan(ctx, resizedBitmap, curStream);

			// get the MediaFile data from db
			MediaFile mf = b2evolution.DB.getMediaFile(src, post);
			if (mf != null) {
