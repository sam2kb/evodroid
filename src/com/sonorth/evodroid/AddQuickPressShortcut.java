...
public class AddQuickPressShortcut extends ListActivity {
		...
		dialogBuilder.setPositiveButton(R.string.add, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if (TextUtils.isEmpty(quickPressShortcutName.getText())) {
	        		...
	        		b2evolution.DB.addQuickPressShortcut(accountIDs[position], quickPressShortcutName.getText().toString());
	        		
	        		if (b2evolution.currentBlog == null) {
	        			try {
							b2evolution.currentBlog = new Blog(accountIDs[position], AddQuickPressShortcut.this);
							b2evolution.DB.updateLastBlogID(accountIDs[position]);
						} catch (Exception e) {
						}
	        		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data != null) {
			switch (requestCode) {
			case 0:
				b2evolutionDB settingsDB = new b2evolutionDB(this);
