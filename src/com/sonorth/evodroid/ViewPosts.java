...
public class ViewPosts extends ListFragment {
	...
	public void refreshPosts(final boolean loadMore) {
		...
		apiArgs.add(b2evolution.currentBlog);
		apiArgs.add(isPage);
	...
	public boolean loadPosts(boolean loadMore) { // loads posts from the db
		if (b2evolution.DB == null)
			b2evolution.DB = new b2evolutionDB(getActivity().getBaseContext());
		if (b2evolution.currentBlog == null) {
			try {
				b2evolution.currentBlog = new Blog(
						b2evolution.DB.getLastBlogID(getActivity().getBaseContext()), getActivity().getBaseContext());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		Vector<?> loadedPosts;
		if (isPage) {
			loadedPosts = b2evolution.DB.loadUploadedPosts(getActivity()
					.getApplicationContext(), b2evolution.currentBlog.getId(),
					true);
		} else {
			loadedPosts = b2evolution.DB.loadUploadedPosts(getActivity()
					.getApplicationContext(), b2evolution.currentBlog.getId(),
	...
	private boolean loadDrafts() { // loads drafts from the db
		...
		Vector<?> loadedPosts;
		if (isPage) {
			loadedPosts = b2evolution.DB.loadDrafts(
					b2evolution.currentBlog.getId(), true);
		} else {
			loadedPosts = b2evolution.DB.loadDrafts(
					b2evolution.currentBlog.getId(), false);
		}
		if (loadedPosts != null) {
	...
	public boolean onContextItemSelected(MenuItem item) {
		Post post = new Post(b2evolution.currentBlog.getId(), selectedID, isPage,
				getActivity().getApplicationContext());
	...
	AsyncTask<Vector<?>, Void, Void> {
		...
		protected Void doInBackground(Vector<?>... args) {
			...
			Object[] params = { b2evolution.currentBlog.getBlogId(),
					...
					b2evolution.currentBlog.getPassword(), numRecords };
			try {
