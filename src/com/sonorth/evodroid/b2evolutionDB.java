...
public class b2evolutionDB {
	...
	private static final String DATABASE_NAME = "b2evolution";
	...
	private static final String CREATE_TABLE_POSTS = "create table if not exists posts (id integer primary key autoincrement, blogID text, "
			...
			+ "latitude real, longitude real, localDraft boolean default 0, uploaded boolean default 0, isPage boolean default 0, b2evo_page_parent_id text, b2evo_page_parent_title text);";
	...
	private static final String CREATE_TABLE_CATEGORIES = "create table if not exists cats (id integer primary key autoincrement, "
			+ "blog_id text, b2evo_id integer, category_name text not null);";
	...
	// add b2evo.net stats login info
	...
	// add b2evo.net flag and version column
	private static final String ADD_DOTCOM_FLAG = "alter table accounts add dotcomFlag boolean default false;";
	private static final String ADD_APP_VERSION = "alter table accounts add appVersion text;";
	...
	public b2evolutionDB(Context ctx) {
	...
	public long addAccount(String url, String blogName, String username,
			...
			boolean runService, int blogId, boolean evonet, String appVersion) {
	...
	public boolean saveSettings(String id, String url, String username,
			...
			String imagePlacement, boolean centerThumbnail,
			boolean fullSizeImage, String maxImageWidth, int maxImageWidthId,
			boolean location, String originalUsername,
	...
	public Vector<Object> loadSettings(int id) {
		...
		Cursor c = db.query(SETTINGS_TABLE, new String[] { "url", "blogName",
				...
				"api_key", "api_blogid", "appVersion", "postFormats",
			...
			for (int i = 0; i < postValues.size(); i++) {
				try {
					...
					if (isPage) {
						...
						values.put("b2evo_page_parent_id",
								thisHash.get("b2evo_page_parent_id").toString());
						values.put("b2evo_page_parent_title",
								thisHash.get("b2evo_page_parent_title").toString());
					} else {
	...
	// categories
	public boolean insertCategory(int id, int b2evo_id, String category_name) {

		ContentValues values = new ContentValues();
		values.put("blog_id", id);
		values.put("b2evo_id", b2evo_id);
	...
	public Vector<String> loadCategories(int id) {
		...
		Cursor c = db.query(CATEGORIES_TABLE, new String[] { "id", "b2evo_id",
	...
	public int getCategoryId(int id, String category) {
		...
		Cursor c = db.query(CATEGORIES_TABLE, new String[] { "b2evo_id" },
