...
public class Blog {
	...
	public Blog(int blog_id, Context ctx) throws Exception{
		...
		Vector<Object> blogVals = b2evolution.DB.loadSettings(blog_id);
		...
		if (blogVals != null) {
			...
			if (blogVals.get(19) != null)
				this.appVersion = blogVals.get(19).toString();
	...
	public void save(Context ctx, String originalUsername) {
		//save blog to db
		b2evolution.DB.saveSettings(String.valueOf(this.id), this.url, this.username, this.password, this.httpuser, this.httppassword, this.imagePlacement, this.centerThumbnail, this.fullSizeImage, this.maxImageWidth, this.maxImageWidthId, this.location, originalUsername, this.postFormats, this.dotcom_username, this.dotcom_password, this.scaledImage, this.scaledImageWidth);
	}
	...
	public int getUnmoderatedCommentCount(Context ctx) {
	    return b2evolution.DB.getUnmoderatedCommentCount(this.id);
	}
